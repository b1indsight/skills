...
1. 只读检查相关文件和现状。
2. 向用户回报计划修改的文件、具体部分、修改方案、影响范围和验证方式。
3. 等待用户明确批准。

本审批门只适用于会改变工作区文件内容的操作。只有获得批准后，才能执行文件写入；若实际文件修改需要实质性扩大已批准范围，必须重新回报方案并再次等待批准。

`git add`、`git commit` 和 `git push` 本身不属于文件修改，不受本审批门约束，可以按照用户任务和仓库维护流程执行。若这些操作或相关 hook 需要或产生额外的文件内容修改，则额外修改仍须先经过审批。
